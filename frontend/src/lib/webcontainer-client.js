...
class WebContainerClient {
  ...
  async startDevServer(onReady) {
    ...
    try {
      ...
      // We check for both .stdout and .output as property names can vary by version
  ...
  async reset() {
    ...
    // Simply remount the clean snapshot
    const tree = await this.loader.assembleTree(); // Get fresh tree from memory chunks
    await this.instance.mount(tree);
    this.status = 'READY';
    this.serverUrl = null;
  }
