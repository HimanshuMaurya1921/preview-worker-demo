...
function shouldPrune(file, fullPath) {
  // Prune non-essential files in node_modules to speed up mounting
...
function walk(dir) {
  ...
  list.forEach(file => {
    ...
    if (stat && stat.isDirectory()) {
      ...
      // Exclude native binaries and other unnecessary files
...
async function main() {
  ...
  const manifest = {
    version: 'v1',
    chunks: [],
    files: {} // filePath -> { chunkIndex, offset, length }
  };
  ...
  for (const file of files) {
    ...
    // If adding this file exceeds the limit, and we already have files, write the chunk
